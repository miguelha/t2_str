...
pointCloud* readPointCloud(const char* file){
    ...
    // declare auxiliary sum variables
    float sumX = 0;
    float sumY = 0;
    float sumZ = 0;
    ...
    for(int i = 0; i < pc->numPts; i++){
        ...
        // this loop will also be used to calculate minimums,
        // maximums and sums
...
int preProcessPointCloud(pointCloud* pc){
    ...
    for(int i = 0; i < pc->numPts; ){
        if(pc->x[i] < 0){
            ...
            memmove(tmpX, pc->x, (i+1)*sizeof(float));
            memmove(tmpX+i, (pc->x)+(i+1), (pc->numPts-i-1)*sizeof(float));

            memmove(tmpY, pc->y, (i+1)*sizeof(float));
            memmove(tmpY+i, (pc->y)+(i+1), (pc->numPts-i-1)*sizeof(float));

            memmove(tmpZ, pc->z, (i+1)*sizeof(float));
            memmove(tmpZ+i, (pc->z)+(i+1), (pc->numPts-i-1)*sizeof(float));
